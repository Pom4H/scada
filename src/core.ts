/** SCADA's declarative vocabulary. No DOM, global registry or mutable project state. */
export type Quality = 'good' | 'stale' | 'bad';
export type Alarm = 'none' | 'warning' | 'trip';
export type Point = { x: number; y: number };
export type Direction = 'left' | 'right' | 'up' | 'down';
export type Kind = 'tank' | 'pump' | 'valve' | 'flowmeter' | 'exchanger' | 'outlet' | 'pressure' | 'temperature';
...
export interface Definition { label: string; width: number; height: number; fields: Record<string, Field>; ports: Record<string, PortSpec>; instrument?: boolean }
...
export const pump = (id: string, props: Base & { rpm?: number; temperature?: number; vibration?: number }) => create('pump', id, props) as Inline;
